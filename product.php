...
<?php
    ...
    $query=mysqli_query($conn,"select count(category_id) from category"); //count row
    $row = mysqli_fetch_row($query);

    $rows = $row[0];
    
    $page_rows = 30; // category row numbers

    $last = ceil($rows/$page_rows); //formula
    ...
    if(isset($_GET['pn'])){
        $pagenum = preg_replace('#[^0-9]#', '', $_GET['pn']); //get page number
    }
    ...
    $limit = 'LIMIT ' .($pagenum - 1) * $page_rows .',' .$page_rows; //algo of pagination
    
    $nquery=mysqli_query($conn,"select * from category $limit"); //select from category with limit based on formula
   ...
   $paginationCtrls = '';
    ...
    if ($pagenum > 1) {
        $previous = $pagenum - 1;
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="https://northwaybicyclestore.tk/home.php?pn='.$previous.'">Previous</a></li></u>'; //links
        
        for($i = $pagenum-4; $i < $pagenum; $i++){
            if($i > 0){
                $paginationCtrls .= '<ul class="pagination justify-content-center"><a class="page-link" href="https://northwaybicyclestore.tk/home.php?pn='.$i.'" >'.$i.'</a></u>'; //links
            }
        }
    }
    
    $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item active"><a class="page-link">'.$pagenum.'</a></li></u>'; //links
    
    for($i = $pagenum+1; $i <= $last; $i++){
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="https://northwaybicyclestore.tk/home.php?pn='.$i.'" >'.$i.'</a></li></u>'; //links
        if($i >= $pagenum+4){
            break;
        }
    }
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="https://northwaybicyclestore.tk/home.php?pn='.$next.'" >Next</a></li></u>'; //links
    }
    }
...
// Check for any mobile device.
if ($detect->isMobile()){ ?>
          <a  href="#" onclick="showhide()">Show / Hide Categories</a>
          <br><br>
          <div id="showhide" style="display: none;"> <?php } ?>
...
<!-- output date when not empty category name -->
...
<?php 
$category_name_click = strtolower(input_($_GET['category_name']));
echo ucwords($category_name_click); ?>
</h3>
...
<?php
    ...
    $category_name = input_(mysqli_real_escape_string($conn,$_GET['category_name']));
    $query=mysqli_query($conn,"select count(inventory_id) from inventory natural join product natural join category where category_name = '$category_name'");
    $row = mysqli_fetch_row($query); //pagination for product
    ...
    if(isset($_GET['pn1'])){
        $pagenum = preg_replace('#[^0-9]#', '', $_GET['pn1']); //formula
    }
    ...
    if ($pagenum < 1) { 
        ...
        $pagenum = $last;  //formula
    }

    $limit = 'LIMIT ' .($pagenum - 1) * $page_rows .',' .$page_rows;
    
    $nquery=mysqli_query($conn,"select * from inventory natural join product natural join category where category_name = '$category_name' $limit"); //select from inventory to know availability of product with limit based on formula
    ...
    if ($pagenum > 1) {
        $previous = $pagenum - 1;
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="'.$_SERVER['PHP_SELF'].'?category_name='.$category_name.'&pn1='.$previous.'">Previous</a></li></u>'; // links
        
        for($i = $pagenum-4; $i < $pagenum; $i++){
            if($i > 0){
                $paginationCtrls .= '<ul class="pagination justify-content-center"><a class="page-link" href="'.$_SERVER['PHP_SELF'].'?category_name='.$category_name.'&pn1='.$i.'" >'.$i.'</a></u>'; // links
            }
        }
    }
    ...
    for($i = $pagenum+1; $i <= $last; $i++){
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="'.$_SERVER['PHP_SELF'].'?category_name='.$category_name.'&pn1='.$i.'" >'.$i.'</a></li></u>'; // links
        if($i >= $pagenum+4){
            break;
        }
    }
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="'.$_SERVER['PHP_SELF'].'?category_name='.$category_name.'&pn1='.$next.'" >Next</a></li></u>'; // links
    }
    }
...
<center>
     <?php 
                 ...
                 if($numrows==0)
                 {
                        echo '<img src = "http://www.datatravelers.com/image/cache/Blog-pixs/google-404-not-found-error-600x600.png" width="230" height="200" ';
                 } //verify if category is exist on database output error image google if not
                 else
                 {
                 $query = "select * from product natural join category where category_name = '$category_name'";
                 $result = mysqli_query($conn, $query) or die(mysqli_error($conn));
                 $numrows = mysqli_num_rows($result);
                 if($numrows==0)
                 {
                        echo '<center><p>There are no product in this category</p><button type="submit" name="submitdelete" onclick="shop()" style="border-radius: 0px !important;" class="btn btn-outline-primary">Back to homepage</a></button></center>';
                 }  //if no result found this back to the homepage
                 else
                 { ?>
</center>
<!-- CONTENT -->
                ...
                <div class="card-body">
                  <h4 class="card-title">
                   ...
                   <p class="card-text">
                    ...
                    <?php if($stock <=0) { echo "<span style='color:red;'>Out of Stock</span>"; } else { echo "In Stock: <span style='color:green;'>$stock</span>"; }; ?>
                  </p>
                </a>
                </div>
                     <!-- ADD TO CART BUTTON -->
                <?php if($stock >0) {  ?>
                <input type="hidden" id="prodproduct_code<?php echo $product_code; ?>" value="<?php echo $product_code; ?>">
                   <center><p class="cart" style="cursor:pointer;"><a href="javascript:;" class="prodproduct_code<?php echo $product_code; ?>">Add to Cart <i class="fa fa-shopping-cart"></i></a>
                   </center>
                   <?php } ?>
                <!-- ADD TO CART BUTTON -->
              </div>
...
<script type="text/javascript">
  ...
  $(document).on('click', '.prodproduct_code<?php echo $product_code; ?>', function(){
    var product_code = $('#prodproduct_code<?php echo $product_code; ?>').val();
    var action = "add";
      $.ajax({
        url:"process.php",
        method:"POST",
...
<!-- IMAGE PREVIEW MODAL -->
          ...
          <!-- /.row -->
<?php echo $paginationCtrls; } //not empty category this is pagination controls ?>
        </div>
        <!-- /.col-lg-9 -->
