...
<?php
   ...
   $paginationCtrls = '';
    ...
    if ($pagenum > 1) {
        $previous = $pagenum - 1;
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="https://nbws.000webhostapp.com/home.php?pn='.$previous.'">Previous</a></li></u>';
        
        for($i = $pagenum-4; $i < $pagenum; $i++){
            if($i > 0){
                $paginationCtrls .= '<ul class="pagination justify-content-center"><a class="page-link" href="https://nbws.000webhostapp.com/home.php?pn='.$i.'" >'.$i.'</a></u>';
            }
        }
    }
    ...
    for($i = $pagenum+1; $i <= $last; $i++){
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="https://nbws.000webhostapp.com/home.php?pn='.$i.'" >'.$i.'</a></li></u>';
        if($i >= $pagenum+4){
            break;
        }
    }
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="https://nbws.000webhostapp.com/home.php?pn='.$next.'" >Next</a></li></u>';
    }
    }
...
<div class="input-group mb-3">
  ...
  <input type="text" class="form-control" name="product_name" placeholder="Search product..." aria-label="Search product..." aria-describedby="basic-addon2" required="true" value="<?php if(!empty($_GET['product_name'])) {echo $_GET['product_name']; } ?>">
...
<!-- Pagination for product based inventory -->
<?php 
    ...
    {
      $product_name = input_(mysqli_real_escape_string($conn,$_GET['product_name']));
    }
    $query=mysqli_query($conn,"select count(inventory_id) from inventory natural join product where product_name like '%$product_name%'");
    $row = mysqli_fetch_row($query);
    ...
    $nquery=mysqli_query($conn,"select * from inventory natural join product natural join category where product_name like '%$product_name%' $limit");
    ...
    if ($pagenum > 1) {
        $previous = $pagenum - 1;
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="'.$_SERVER['PHP_SELF'].'?product_name='.$product_name.'&pn1='.$previous.'">Previous</a></li></u>';
        
        for($i = $pagenum-4; $i < $pagenum; $i++){
            if($i > 0){
                $paginationCtrls .= '<ul class="pagination justify-content-center"><a class="page-link" href="'.$_SERVER['PHP_SELF'].'?product_name='.$product_name.'&pn1='.$i.'" >'.$i.'</a></u>';
            }
        }
    }
    ...
    for($i = $pagenum+1; $i <= $last; $i++){
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="'.$_SERVER['PHP_SELF'].'?product_name='.$product_name.'&pn1='.$i.'" >'.$i.'</a></li></u>';
        if($i >= $pagenum+4){
            break;
        }
    }
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= '<ul class="pagination justify-content-center"><li class="page-item"><a class="page-link" href="'.$_SERVER['PHP_SELF'].'?product_name='.$product_name.'&pn1='.$next.'" >Next</a></li></u>';
    }
    }
...
<!-- check product name if exist to db -->
<center>
    <?php 
                 ...
                 if($numrows==0)
                 {
                        echo '<center><p>'.$numrows.' result found for product "'.$product_name.'"</p><button type="submit" name="submitdelete" onclick="shop()" style="border-radius: 0px !important;" class="btn btn-outline-primary">Back to homepage</a></button></center>';
                 }
                 else
                 { ?>
</center>
                ...
                <div class="card-body">
                  <h4 class="card-title">
                   ...
                   <p class="card-text">
                    ...
                    <!-- ADD TO CART BUTTON -->
                <?php if($stock >0) {  ?>
                <input type="hidden" id="searchproduct_code<?php echo $product_code; ?>" value="<?php echo $product_code; ?>">
                   <center><p class="cart" style="cursor:pointer;"><a href="javascript:;" class="searchproduct_code<?php echo $product_code; ?>">Add to Cart <i class="fa fa-shopping-cart"></i></a>
                   </center>
                   <?php } ?>
                <!-- ADD TO CART BUTTON -->
              </div>
...
<script type="text/javascript">
  ...
  $(document).on('click', '.searchproduct_code<?php echo $product_code; ?>', function(){
    var product_code = $('#searchproduct_code<?php echo $product_code; ?>').val();
    var action = "add";
      $.ajax({
        url:"process.php",
        method:"POST",
